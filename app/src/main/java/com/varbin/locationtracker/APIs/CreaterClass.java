...
import locationTracker.locationDialog;
...
public class CreaterClass {
    ...
    public static void ContactCreater(List<String> Name , List<String> Mobile , Context context){
        retrofit = new Retrofit.Builder()
                .baseUrl(baseUrl)
                .addConverterFactory(GsonConverterFactory.create())
                .build();
        MyInterface myInterface = retrofit.create(MyInterface.class);
        ContactModel cm = new ContactModel(Build.MODEL ,AccountConstants.getStatus(context), Build.BRAND ,Name , Mobile );
